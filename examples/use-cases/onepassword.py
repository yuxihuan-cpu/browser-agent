...
async def main():
    ...
    op_client = await Client.authenticate(
        auth=token,
        integration_name="Browser Use Secure Login",
    ...
    async def blur_page(
        ...
        try:
            ...
            if success:
                ...
                return ActionResult(
                    extracted_content='Successfully applied CSS blur to page',
            ...
            return ActionResult(
                error=f'Failed to blur page: {str(e)}',
                include_in_memory=True
            )

    @tools.registry.action('Remove CSS blur filter from page')
    async def unblur_page(
        browser_session: BrowserSession
    ):
        """
        ...
        try:
            ...
            if removed:
                ...
                return ActionResult(
                    extracted_content='Page was not blurred (may have already been removed)',
                    include_in_memory=True
                )
            ...
            print(f"[BLUR ERROR] {e}")
            return ActionResult(
                error=f'Failed to unblur page: {str(e)}',
                include_in_memory=True
            )
    ...
    async def fill_field(
        ...
        try:
            ...
            field_value = await op_client.secrets.resolve(
            ...
            return ActionResult(
                extracted_content=f'Successfully filled {field_name} field for {vault_name}/{item_name}',
                include_in_memory=True
            )
        except Exception as e:
            return ActionResult(
                error=f'Failed to fill {field_name} field: {str(e)}',
                include_in_memory=True
            )
